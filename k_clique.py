...
def find_cliques_at_node(node_id, adj_list):
    ...
    while len(queue) > 0:
        ...
        if (not set(curr_clique).issubset(set(local_max_clique)) and 
            not set(curr_clique).issubset(set(max_clique)) and len(filter(
            ...
            expanded_nodes = filter(lambda i: (i > max(curr_clique) and
...
def find_cliques(adj_lists, min_size=4):
    ...
    for nid in adj_lists_pruned: 
        ...
        if (len(clique) >= min_size and
            len(filter(set(clique).issubset, map(set, cliques)))==0):
...
def run():
    ...
    for nid in node_ids: 
        evl = evaluation([], nid)
        evl.circle_list_detected = map(list, find_cliques(get_adj_list(nid)))
